...
pygame.display.pygame.display.set_caption("Snake Game")
...
def show(self):
    for i in range(self.length):
        if not i == 0:
            pygame.draw.rect(display, snake_color, (self.history[i][0],
            self.history[i][1], self.w, self.h))
...
def show_score():
        font = pygame.font.dysFont("Copper Gothic Bold", 20)  
        text = font.render("Score: " + str(score), True, snake_color)
        display.pygame.Surface.blit(scale, scale)
